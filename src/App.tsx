...
function App() {
  ...
  const [winningNumbers, setWinningNumbers] = useState<LotteryNumbers | null>(null);
        ...
        {tickets.length > 0 && (
          <div className="bg-white/5 rounded-xl p-6 backdrop-blur-sm mb-8">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                disabled={winningNumbers != null}
                onClick={runLottery}
                className="bg-purple-600 hover:bg-purple-700 px-6 py-3 rounded-lg flex items-center gap-2 text-lg font-semibold"
              >
                <Play />
                Draw Numbers
              </button>
            ...
            <div className="space-y-4">
              {tickets.map((ticket) => (
                <div key={ticket.id} className="flex items-center justify-between bg-white/5 p-4 rounded-lg">
                  <div className="flex gap-2">
                    {ticket.white.map((num, i) => (
                      <div key={i} className="w-10 h-10 rounded-full bg-white text-blue-900 flex items-center justify-center font-bold text-lg">
                        {num}
                      </div>
                    ))}
